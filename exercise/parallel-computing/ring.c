...
int main(int argc, char *argv[]){
    ...
    // Take care of the special case when you are the first process to prevent deadlock.
    if(world_rank != 0){
    ...
    // This makes sure that at least one MPI_Send is initialized all MPI_Recvs (again, to prevent deadlock)
    if(world_rank == 0){
